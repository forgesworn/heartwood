...
pub fn from_nsec_bytes(nsec_bytes: &[u8; 32]) -> Result<TreeRoot, HeartwoodError> {
    ...
    let tree_root_secret: [u8; 32] = result.into_bytes().into();
    create_tree_root(tree_root_secret)
}
...
pub fn from_mnemonic(mnemonic: &str, passphrase: Option<&str>) -> Result<TreeRoot, HeartwoodError> {
    ...
    let private_key_bytes: [u8; 32] = child.to_bytes();

    create_tree_root(private_key_bytes)
}
